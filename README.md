...
- 트레이 아이콘에서 상태 확인, 설정 변경, 시작프로그램 토글 제공
- 중복 실행 방지
...
| 항목 | 설명 |
|------|------|
| 상태 | 현재 CPU/RAM 수치와 조치 여부 |
| 마지막 확인 | 가장 최근 확인 시각 |
| 모드 | `active` 또는 `dry-run` |
| 마지막 조치 | 가장 최근 조치 내용 |
| 조치 횟수 | 이번 실행 세션의 총 조치 수 |
| 일시정지 / 다시 시작 | 감시 일시 중단 및 재개 |
| 지금 확인 | 즉시 한 번 확인 실행 |
| 설정 열기 | 메모장으로 `calmkeeper.config.json` 열기 |
| 설정 다시 읽기 | 재시작 없이 설정 즉시 반영 |
| 로그 열기 | 메모장으로 `calmkeeper.log` 열기 |
| 시작프로그램 | Windows 시작 시 자동 실행 등록/해제 토글 |
| 종료 | CalmKeeper 종료 (우선순위 복원 포함) |
...
트레이 메뉴 → **시작프로그램** 항목을 클릭하면 등록/해제를 바로 전환할 수 있습니다.
